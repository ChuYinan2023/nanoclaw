...
// Message queue: buffers messages from the moment the WebSocket is created,
// preventing the race where the server sends session/history frames during
// the connection handler before test code calls waitForMessage().
interface MsgQueue {
  buffered: any[];
  waiter: ((msg: any) => void) | null;
}
const queues = new WeakMap<WebSocket, MsgQueue>();
...
async function connectWs(
  ...
  const q: MsgQueue = { buffered: [], waiter: null };
  queues.set(ws, q);
  ws.on('message', (data) => {
    const msg = JSON.parse(data.toString());
    if (q.waiter) {
      const resolve = q.waiter;
      q.waiter = null;
      resolve(msg);
    } else {
      q.buffered.push(msg);
...
function waitForMessage(ws: WebSocket): Promise<any> {
  ...
  if (q.buffered.length > 0) {
    return Promise.resolve(q.buffered.shift()!);
  }
